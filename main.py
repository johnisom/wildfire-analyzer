...
def plot_fire_causes(keys=('lower48',), year_start=None, year_end=None):
  ...
  plot_causes_of_fires_by_state(fires_df, keys, plot_title)
  plt.show()

def plot_fire_counts(keys=('lower48',), year_start=None, year_end=None):
  fires_df = get_fires_dataframe()
  year_start = max(fires_df.fire_year.min(), year_start or -inf)
  year_end = min(fires_df.fire_year.max(), year_end or inf)
  fires_df = fires_df[fires_df['fire_year'].between(year_start, year_end)]
...
def plot_fire_area_burned(keys=('lower48',), year_start=None, year_end=None):
  fires_df = get_fires_dataframe()
  year_start = max(fires_df.fire_year.min(), year_start or -inf)
  year_end = min(fires_df.fire_year.max(), year_end or inf)
  fires_df = fires_df[fires_df['fire_year'].between(year_start, year_end)]
...
def plot_everything(keys=('lower48',), year_start=None, year_end=None):
  ...
  add_fire_count_to_counties(counties_geo_df, fires_df)
  add_acres_burned_to_counties(counties_geo_df, fires_df)
  fire_causes_title = f'Number of reported wildfires in the US by fire cause between {year_start} and {year_end}'
  ...
  plot_causes_of_fires_by_state(fires_df, keys, fire_causes_title)
